...
// Traitement sur les RH
		if( this.DataSetManager().DataSet( 'rh' ) ){
			...
			this.get( 'oLayout' ).on( 'layout:intialized', function() {
				...
				nodeListRHCoutTotMens = Y.all('.rh_typerecrut select, .rh_catperso select, .rh_coutmenscharg input, .rh_pi_on select, .rh_montantprime input, .rh_quotite select');
				...
				nodeListRHImpSCSPTot = Y.all('.rh_typerecrut select, .rh_couttotmens input, .rh_impscspduree input, .rh_catperso select, .rh_coutmenscharg input, .rh_pi_on select, .rh_montantprime input, .rh_quotite select, .rh_impscspmontant input');
				...
				nodeListRHImpC2Tot = Y.all('.rh_typerecrut select, .rh_impc2 select, .rh_typec2 input, .rh_impc2duree input, .rh_couttotmens input, .rh_catperso select, .rh_coutmenscharg input, .rh_pi_on select, .rh_montantprime input, .rh_quotite select, .rh_impc2montant input');
				...
				nodeListRHImpC1Tot = Y.all('.rh_typerecrut select, .rh_impc1 select, .rh_typec1 input, .rh_impc1duree input, .rh_couttotmens input, .rh_catperso select, .rh_coutmenscharg input, .rh_pi_on select, .rh_montantprime input, .rh_quotite select, .rh_impc1montant input');
				...
				nodeListRHCoutTot = Y.all('.rh_typerecrut select, .rh_impc1 select, .rh_typec1 input, .rh_impc1duree input, .rh_impc2 select, .rh_typec2 input, .rh_impc2duree input, .rh_couttotmens input, .rh_catperso select, .rh_coutmenscharg input, .rh_pi_on select, .rh_montantprime input, .rh_quotite select, .rh_impc1montant input, .rh_impc2montant input, .rh_impscspduree input, .rh_impscspmontant input');
...
// Traitement sur les stagiaires
	...
	updateCalcRHCoutTotMens: function(event){
		...
		if(event.target.ancestor(".field").hasClass('rh_pi_on') || event.target.ancestor(".field").hasClass('rh_catperso')){
			if (aRecord.rh_pi_on == 10) {
				rh_catperso=aRecord.rh_catperso;
				rh_montantprime = parseFloat(arh_prime[rh_catperso]);
				this._setFieldValue( 'rh', 'rh_montantprime', rh_montantprime );
			}else {
				rh_montantprime = parseFloat(0);
				this._setFieldValue( 'rh', 'rh_montantprime', Math.round(0) );
			}
		}else {
			rh_montantprime = aRecord.rh_montantprime;
		}
