...
class Copilote_Library_Depense
{
	 ...
	 * @var string
	 */
	protected $_tableName = "cplt_dpns_data" ;
	...
	public function getDemande()
	{
		if ( is_null( $this->_demande ) ) {
			$this->_demande = new Copilote_Library_Demande( $this->getAttribute( "id_demande" ) ) ;
		}
	...
	{
		$db = Core_Library_Account::GetInstance()->GetCurrentProject()->Db() ;
		return (float) $db->fetchOne( "SELECT SUM( sta_impscspmontant ) FROM cplt_sta_data WHERE id_depense = ?", $this->_id ) ;
	}
	...
	{
		$db = Core_Library_Account::GetInstance()->GetCurrentProject()->Db() ;
		return (float) $db->fetchOne( "SELECT SUM( COALESCE(sta_impc1montant, 0) + COALESCE(sta_impc2montant, 0) ) FROM cplt_sta_data WHERE id_depense = ?", $this->_id ) ;
	}
